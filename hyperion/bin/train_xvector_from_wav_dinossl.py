...
def init_data(partition, rank, num_gpus, **kwargs):
    ...
    if dinossl_args["dinossl"]:
        dataset = AD(**ad_args,dinossl_chunk_len_mult=dinossl_args["dinossl_chunk_len_mult"], dinossl_n_chunks=dinossl_args["dinossl_local_crops_number"] + 2, dinossl_reduce_overlap_prob=float(dinossl_args["dinossl_reduce_overlap_prob"]))
    else:
        dataset = AD(**ad_args)
